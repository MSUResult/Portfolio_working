...
const getSeoUrl = (path: string) => {
  const baseUrl = "http://port.saharanpurprice.in";
...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: metadata.title,
      description: metadata.summary,
      url: `http://port.saharanpurprice.in/blogs/${decodedSlug}`,
      images: [
        {
          url: seoImageUrl,
          width: 1200,
...
const SingleBlogPage = async ({ params }) => {
  ...
  const articleJsonLd = {
    ...
    "author": {
      "@type": "Person",
      "name": authorName,
      "url": metadata.author?.url || "http://port.saharanpurprice.in/about",
    },
    "publisher": {
      "@type": "Organization",
      "name": "Shivansh Portfolio",
      "logo": { "@type": "ImageObject", "url": "http://port.saharanpurprice.in/icon.png" }
    },
    "description": metadata.summary,
  };
